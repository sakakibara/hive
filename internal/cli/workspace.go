...
func init() {
	...
	workspaceCmd.AddCommand(wsShellCmd)
}
...
var wsShellCmd = &cobra.Command{
	Use:       "shell <shell>",
	Short:     "Generate shell configuration for h and hi functions",
	Long:      "Print shell functions for the given shell.\nAdd to your shell config:\n  fish: hive workspace shell fish | source\n  bash: eval \"$(hive workspace shell bash)\"\n  zsh:  eval \"$(hive workspace shell zsh)\"",
	Args:      cobra.ExactArgs(1),
	ValidArgs: []string{"fish", "bash", "zsh"},
	RunE:      runWsShell,
}

func runWsShell(cmd *cobra.Command, args []string) error {
	output, err := workspace.ShellInit(args[0])
	if err != nil {
		return err
	}
