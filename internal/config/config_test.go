...
func TestBackupDir_ICloudMode(t *testing.T) {
	cfg := &Config{
		Paths: PathsConfig{
			Projects: "~/Projects",
			Life:     "~/Life",
	...
	want := filepath.Join(home, "Library/Mobile Documents/com~apple~CloudDocs/workspace", "backups")
	got := cfg.BackupDir()
	if got != want {
...
func TestBackupDir_LocalMode(t *testing.T) {
	cfg := &Config{
		Paths: PathsConfig{
			Projects: "~/Projects",
			Life:     "~/Life",
